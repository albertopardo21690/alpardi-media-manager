...
| `.claude/skills/` | 9 skills reales (envoltorios finos sobre la CLI real, nunca reimplementan lógica), `/plex-apply`/`/plex-rollback` con `disable-model-invocation: true`. Verificado con comandos oficiales reales: `claude doctor` (config válida), `claude mcp list` (servidor MCP detectado, pendiente de aprobación interactiva de Alberto), `claude -p` (CLAUDE.md cargado y respondido con exactitud; las 7 skills normales aparecen listadas, `/plex-apply` y `/plex-rollback` NO aparecen -- confirma en vivo que su descripción ni siquiera llega al contexto del modelo) | 9 `SKILL.md` |
